...
public class att_activity extends AppCompatActivity {
    Toolbar toolbar_n;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseRecyclerOptions<notes_model> options = new FirebaseRecyclerOptions.Builder<notes_model>()
                .setQuery(FirebaseDatabase.getInstance().getReference()
                        .child("Attendance"),notes_model.class).build();
        adapter=new notes_adapter(options);
        recyclerView.setAdapter(adapter);

        DatabaseReference ref = FirebaseDatabase.getInstance().getReference().child("Project Mentor List");
